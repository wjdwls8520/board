...
public class BoardController {
    ...
    public String fileupload(@RequestParam("image") MultipartFile file, HttpServletRequest request, Model model) {
        ...
        String savefilename = fn1 + t + fn2;  // 저장파일 이름 생성
        String uploadPath = path + "/" + savefilename;  // 저장 경로와 파일 이름 완성
        try {
            file.transferTo(new File(uploadPath));  // 파일업로드
    ...
    public String insertBoard(
          ...
          @RequestParam("uploadImage") MultipartFile file,    // 이미지 보낼때 필수.
            Model model) {
        String url = "board/insertBoard";
        ...
        if(result.hasFieldErrors("title")) {
            ...
            try {
                file.transferTo(new File(uploadFilePath));  // 파일업로드
            } catch (IllegalStateException e) {
                e.printStackTrace();
            } catch (IOException e) {
                e.printStackTrace();
