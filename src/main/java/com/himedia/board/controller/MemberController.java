...
public class MemberController {
    ...
    public String login(
        ...
        String url = "/member/loginForm";
            ...
            // 비어있는 입력에 대한 에러처리
            if(result.hasFieldErrors("userid")) model.addAttribute("msg", result.getFieldError("userid").getDefaultMessage());
            else if(result.hasFieldErrors("pwd")) model.addAttribute("msg", result.getFieldError("pwd").getDefaultMessage());
            else {
                // 정상 로그인
    ...
    @PostMapping("/join")
    public String join(
            @ModelAttribute("dto") @Valid MemberDto memberdto, BindingResult result,   // 벨리데이션 체크하는 녀석들


            @RequestParam(value="reid", required = false) String reid,                           // dto에 없기때문에 따로 파라미터로 값을 받음
            @RequestParam(value="pwd_check", required = false) String pwd_check,        // 리퀘스트파람을 쓸때 빈값이 오면 에러남, 그렇기에 대처를   
                                                                                                                         // 해야함  ex)  value="pwd_check", required = false 
                                                                                                                         // 꼭 빈값이 아니여야만 한게 아니어도 된다는 뜻
            Model model
    ) {
        ...
        if(result.hasFieldErrors("userid")) model.addAttribute("msg", result.getFieldError("userid").getDefaultMessage());
        else if( !memberdto.getUserid().equals(reid)) model.addAttribute("msg", "아이디 중복검사를 진행하세요.");
        else if(pwd_check == null || !pwd_check.equals(memberdto.getPwd())) model.addAttribute("msg", "패스워드 확인이 일치하지 않습니다.");
        else if( result.hasFieldErrors("name")) model.addAttribute("msg", result.getFieldError("name").getDefaultMessage());
        else if( result.hasFieldErrors("email")) model.addAttribute("msg", result.getFieldError("email").getDefaultMessage());
        else if( result.hasFieldErrors("phone")) model.addAttribute("msg", result.getFieldError("phone").getDefaultMessage());
        else {
            ms.insert( memberdto );
            model.addAttribute("msg", "회원가입이 완료되었습니다. 로그인하세요.");
    ...
    public String kakaoLogin(@RequestParam("code") String code, HttpServletRequest request) throws IOException {
        ...
        while ((input=br.readLine()) != null){ // 응답 수신
            sb.append(input);   // 수신내용의 누적
            System.out.println(input);  // 수신 내용을 그때 그떄 출력
        }

        /*{
            "access_token":"qhjmz3zQ1wKnP-qqUPP8TywSFey5tRGjAAAAAQoNG5oAAAGYQMeMTujqOP6o1CZo",
            "token_type":"bearer",
            "refresh_token":"KCRb77zvp_nVyMiYaJBRwSEY1_MQGP79AAAAAgoNG5oAAAGYQMeMRujqOP6o1CZo",
            "expires_in":21599,
        ...
        System.out.println("어세스토큰.어세슨토큰 : " + oAuthToken.getAccess_token());
    ...
    /*{
        "id":4365952547,
        "connected_at":"2025-07-25T08:51:16Z",
        "properties":{
             "nickname":"김정진","profile_image":"http://k.kakaocdn.net/dn/o0d6V/btsPt0b4ZQT/Rh2OFTv3hJgOG56Z25wsa0/img_640x640.jpg",
             "thumbnail_image":"http://k.kakaocdn.net/dn/o0d6V/btsPt0b4ZQT/Rh2OFTv3hJgOG56Z25wsa0/img_110x110.jpg"
         },
         "kakao_account":{
            "profile_nickname_needs_agreement":false,
            "profile_image_needs_agreement":false,
            "profile":{
                "nickname":"김정진","thumbnail_image_url":"http://k.kakaocdn.net/dn/o0d6V/btsPt0b4ZQT/Rh2OFTv3hJgOG56Z25wsa0/img_110x110.jpg",
                "profile_image_url":"http://k.kakaocdn.net/dn/o0d6V/btsPt0b4ZQT/Rh2OFTv3hJgOG56Z25wsa0/img_640x640.jpg",
